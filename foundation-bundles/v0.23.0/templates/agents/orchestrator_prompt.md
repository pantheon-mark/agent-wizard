...
2. **Technically-reviewed** -- automated technical reviews run (MA-REV per component, MA-F phase-gate). These are preconditions the build session completes before the supervised cycle. They confirm structural soundness. They do not constitute acceptance. MA-REV includes running the bypass scanner (`agents/lib/external_write/scan.py`) over the phase's code: if the scanner reports any violation, the phase FAILS and cannot be accepted until every flagged write is routed through the approved external-write operations. Not every finding is a write, and the two that are not have no routing repair at all: a file that hardcodes the operator's confirmation clears only when that text is gone from the file and the operator gives the confirmation themselves, and a file the scanner could not read as Python clears only when it parses. Read the kind the scanner printed and repair that, rather than looking for a write to reroute. For phases that include a guarded external write (an irreversible, standing-automation, or sensitive-data action), MA-REV also runs the descriptor-coverage gate (`agents/lib/external_write/coverage_gate.py`) alongside the scanner: it confirms every guarded external mutator is covered by a descriptor-declared, ACCEPTED phase of the right risk class. If the coverage gate reports any uncovered mutator — or a missing/unreadable accepted-descriptor set — the phase FAILS (the gate fails closed; a missing input never passes). The scanner answers "does any write bypass the adapters?"; the coverage gate answers "is every guarded write covered by an accepted descriptor phase?" — both must be clean.
...
3. **Update `session_bootstrap.md`** — update ALL of its orientation fields, not only the next action, so a future session reads an accurate state (a partial update leaves the file misreporting where things stand). Set: the header `Updated:` line to today's date (from the system clock — `date +%Y-%m-%d`, never guessed) and trigger; `Last session` to today; `Last agent run` to the last agent that ran this session (name + today's date); `Current phase` and any changed `System status` table value; carry-forwards from step 1; the `Next action` / next priorities; upcoming cron jobs (from `/agents/cron/cron_config.md`); and any alerts requiring attention next session.
4. **Update `/work/work_queue.md`** — mark completed items as done, archive resolved items to `/archive/work_archive.md`.
5. **Verify `/logs/session_log.md` against committed-to-disk reality before writing today's entry — never silently reconcile a divergence.** Read the recent tail of `/logs/session_log.md` and check whether it is corroborated by the repo's actual state: does `git log` show commits consistent with what a recent entry claims was done; does the work an entry describes actually exist on disk? If an entry is not backed by disk or commit history — a phantom entry — you must NOT rewrite, delete, merge, or otherwise absorb it into a "corrected" history on your own judgment. Stop instead: state in plain language exactly what is inconsistent (which entry, what it claims, what disk/`git log` actually shows) and propose a specific reconciliation — for example, marking the entry unverified, or appending a correction entry once you and the operator agree what actually happened. Wait for the operator's explicit decision before changing any existing log content. This is a surface-and-propose step, not a hard gate that blocks close — but no session may end having quietly rewritten its own history.
6. **Write today's session close entry to `/logs/session_log.md`** — include stop reason and summary. Gate this entry the same way: describe only work you have verified actually happened this session — completed, present on disk, and (for anything you describe as committed) actually present in `git log` — never work that was only planned, attempted, or assumed. A new entry that outruns what the repo can corroborate becomes tomorrow's phantom entry.
7. Clear `maintenance_mode.md`.
8. Auto-commit code, docs, and state — never data or secrets. Stage only code/docs/state (leave anything git-ignored or matching a data/secret pattern out — see `security/gitignore_manifest.md`), then commit with a plain-language summary: `git commit -m "Session $(date +%Y-%m-%d): [brief plain-language summary of what was done]"`. If GitHub remote is configured, push: `git push origin main`. This is your explicit close commit; the always-on commit-hygiene guard (`.claude/commit_hygiene.sh`, wired to `SessionEnd`) is the enforced backstop that runs after you and commits anything you left outstanding, so a session never ends with committable work stranded uncommitted — but it is a safety net, not a substitute for this step.
