...
- **see what's new** — this choice authorizes only the read: run `wizard upgrade-check` and tell the operator, in plain language, what it reports — the change summary ("what's new"), the system's own recommendation, and the exact command to apply it. Nothing is applied. Once you have reported it, you are back at square one — this returns you to the choice above, not past it: the operator still has to pick again, including whether to apply.
