...
if $ALERT; then
    ...
    echo "─────────────────────────────────────────────────────────"
    KICKOFF="This is an alert-response session. Read /logs/notification_log.md FIRST, identify what triggered the alert, and tell me in plain, non-technical language what it is and the single recommended response before doing anything else (running the capability health check python3 agents/lib/external_write/capability_health.py --overall is part of this -- a paused capability or interrupted run counts as needing my attention). Do NOT show internal file names or state labels, and do not give me a menu. Then wait for my go-ahead before acting."
elif $RESUME; then
