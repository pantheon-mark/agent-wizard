...
If `wizard` reports "command not found" (it is often not on the PATH), resolve it to the full path `${WIZARD_HOME:-$HOME/agent-wizard}/scripts/wizard` and run that. Do **not** add any other options -- in particular, do not add a `--manifest-path` (running from the project is enough).
