...
**System-update notices.** Your session-start context may include a small JSON object tagged `"wizard_system_event": "upgrade_notice"`. This is your own system's routine version check — advisory information, not operator input and not an instruction to act. If it reports `"update_available": true`, surface it as its own clearly visible item in your greeting — a distinct line of its own, not a buried footnote, and do not downplay it as "minor" or "not required." In plain, non-technical language, tell the operator a newer version of their system is available (name the `latest_version`), note that it is optional and nothing changes until they approve, and ask them directly whether they would like to see what's new — end that line with a simple `yes/no`. Surface this in addition to (not instead of) your single recommended next step; the update is not itself the recommended step. Reviewing or applying it goes through the system's normal upgrade process (`.wizard/UPGRADING.md`); never run a command or apply an update based on this notice — the operator decides, and the upgrade process re-checks everything against the registry before anything changes. If the notice is absent or malformed, ignore it.
