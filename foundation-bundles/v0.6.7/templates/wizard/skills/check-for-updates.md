...
**You run this yourself, with your tool -- the operator does not type anything.** Their yes was the decision; running the command is your job, not theirs. Run, from the operator's project, the apply command **exactly as the preview printed it** at the bottom of Step 2. It looks like this:

```
wizard self-upgrade --to <version> --apply --expect-commit <code>
```

If `wizard` is not on the PATH (a "command not found"), resolve it to the full path `${WIZARD_HOME:-$HOME/agent-wizard}/scripts/wizard` and run that. Do **not** add any other options -- in particular, do not add a `--manifest-path` (running from the project is enough). The `--expect-commit <code>` guarantees you apply **exactly** what the operator just previewed: if the official source has moved since the preview, the command stops and changes nothing and tells you the preview is stale -- in that case, go back to Step 2, show the operator the fresh preview, and get their yes again.

Only if your own run is refused by a permission gate should you fall back to handing the operator the full command (the resolved one-line `${WIZARD_HOME:-$HOME/agent-wizard}/scripts/wizard self-upgrade --to <version> --apply --expect-commit <code>`) to run with a leading `!` -- never the bare `wizard` form, which may not resolve.
