...
If the plan step instead reports **`TOOLKIT_BEHIND`** (your update tool is behind and does not yet carry this version), that is expected and is **not** an error and **not** a reason to wait. Tell the operator the update is available, then go straight to Step 3 -- the Step 3 command refreshes the tool and applies in one step.

## Step 3 -- apply (only after an explicit yes)

```
wizard self-upgrade --to <version> --apply
```
