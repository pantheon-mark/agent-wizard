...
to the bare kernel dispatch module -- `from external_write import adapters`
-- followed by using `run_operation`, the one legitimate CAPABILITY-facing
entrypoint that module exports. `adapters` (bare) is NOT an adapter-PROFILE
module: `"adapters".startswith("adapters_")` is False (the prefix requires
the trailing underscore + a non-empty suffix), and it is not the registry
