...
class SupervisedCopyTargetSourceGuardTest(unittest.TestCase):
    ...
    def test_bundle_next_phase_matches_dev_home(self):
        ...
        RE-SYNCED AGAIN at the v0.15.0 bundle cut (Cut 1.1, Task A3, F-71). Task A3 added the
        "What this capability's own test should (and should not) cover" guidance section to
        the DEV-HOME wizard/skills/next-phase.md, steering a capability author away from
        writing an ambient-pause-state-dependent test and toward the new hermetic fixture
        (external_write.lifecycle_test_fixtures). v0.14.0 was already-RELEASED and
        byte-immutable at the time, so it could not be edited to carry this — the guard was
        marked expectedFailure until Cut 1.1 cut its own bundle. The v0.15.0 cut ported the
        current wizard/skills/next-phase.md into the bundle byte-for-byte (verified via cmp),
        restoring dev-home == latest-bundle; the marker was removed accordingly. Per the same
        self-clearing convention documented above: if a future slice edits the dev-home copy
        again before its own bundle cut ships, this assertion will fail again and must be
        re-marked expectedFailure with a fresh note, not silently patched around."""
        self.assertEqual(
