---
description: "Set up a new capability that was not in the original plan. Use when the operator asks for something the plan does not cover, wants the system to also do something new, says 'can it also...', 'I want it to...', 'add a feature', 'set up something new', or asks for off-plan work; when the next-phase skill stops because the request does not match the plan; or when the safety guard halts an off-plan action and sends the operator here. Turns an off-plan, operator-originated request into a proper planned step, then hands off to the next-phase skill to build it. Not for building a phase that is already in the plan — use next-phase for that. Not for rebuilding a paused or migration-flagged existing capability — that is a technical repair on something that already has a plan and a purpose, not a new one; use rebuild-paused-capability for that."
---
...
**It does not:** build the capability here, and it does not turn anything on. Setting a capability up in the plan does not authorize it to touch anything live. Building, a supervised trial run on a copy, and your acceptance all come afterward, through the next-phase skill. Defining a capability and granting it live use are deliberately two separate moments. It also does not bring back an existing capability that is switched off and queued — new capability only. Whatever put it there (an upgrade found a safety problem, its code changed after approval, or the operator took their approval back), that is not a redesign, and it goes through the rebuild-paused-capability skill instead.
...
**Also check for a pending migration.** If `agents/handoffs/pending_migrations.json` exists and is non-empty, something the operator already has is switched off and waiting. Each entry names the mechanism (its `mechanism_id`), what sort of entry it is (`kind`), the `reason` it is off, and a `suggested_next_step`. **Read those three fields rather than assuming the cause** — entries arrive for more than one reason and they do not all mean the same thing:

- **An upgrade found a safety problem.** An existing mechanism no longer follows a safety rule (see `operating_discipline.md`), so it was safe-paused rather than left running unsafely or broken outright. Its code needs rebuilding.
- **`kind: acceptance_stale` — the code moved out from under an approval.** A capability's implementation changed after it was approved for live use, so the approval was switched off pending a fresh trial. This does not by itself mean anything is wrong with the code.
- **`kind: acceptance_repudiated` — the operator took their approval back.** They withdrew it themselves. Nothing about the capability's code changed, and there may be nothing wrong with it at all — they simply do not want it live, or did not recognise the approval that was on record.

The entry's own `kind` and `reason` say which. Do not tell the operator their code has a problem when the entry says they withdrew an approval. This list is not exhaustive — other kinds exist; when you meet one you do not recognise, read its `reason` and `suggested_next_step` rather than assuming it is like one of these.

**This is an existing capability, not a new one — do not interview about it here.** A pending-migration entry is about something the operator already has, not a fresh "what should this help with?" question, so this skill's own interview (Step B onward) is the wrong path for it. If the operator hasn't already brought it up, mention it plainly — **in the entry's own terms** — and hand off. For a safety-pause or a changed-code entry, for example:

> Something you already had is switched off, waiting to be rebuilt safely. There's a dedicated flow for exactly that, so we don't need to redesign it from scratch here.

For an entry the operator's own withdrawal created, that framing would be wrong — nothing is broken. Say what actually happened:

> You took your approval for this one back, so it's switched off. If you want it running again it needs a fresh trial and a fresh approval from you. If you don't, leaving it as it is, is a complete answer.

Then, **unless the operator says they do not want the capability back**, hand off to the **rebuild-paused-capability** skill. What that flow does not change is the capability's business purpose, so the rebuild keeps the same id as the entry's `mechanism_id` rather than declaring a new one. How much of its wiring actually needs to change depends on why it is off, and that flow dispatches on the entry's `kind` to decide: a safety-paused capability needs real repair, whereas one the operator simply withdrew goes straight to a fresh trial and a fresh approval with nothing changed. Either way the entry will close itself automatically the moment the capability is accepted for live use: you never edit `pending_migrations.json` by hand, and you never need to remember to remove the entry yourself. A capability the operator has decided they do not want back is not stranded either — the entry stays, visible, and it stays off.

**Also run the capability health probe.** `pending_migrations.json` only names a broken capability if an upgrade already found it and queued it. That queue can miss one — a reconcile pass that ran before this capability existed, or that scoped over a different part of the project. So do not rely on the queue alone: also run `capability_health` (the same deterministic check named in `operating_discipline.md`) over the operator's existing capabilities before you propose anything. If it reports a capability as red with no matching `pending_migrations.json` entry, hand off to the rebuild-paused-capability skill the same way — do not treat it as a candidate for this skill's own interview. A capability the operator would touch is never left stranded just because no queue entry happened to catch it.
