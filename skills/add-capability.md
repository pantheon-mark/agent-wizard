...
**Also check for a pending migration.** If `agents/handoffs/pending_migrations.json` exists and is non-empty, an upgrade previously found an existing mechanism that no longer follows a safety rule (see `operating_discipline.md`) and safe-paused it rather than leaving it running unsafely or breaking it outright — each entry names the paused mechanism (its `mechanism_id`), why, and what changed.

**This is an existing capability, not a new one — do not interview about it here.** A pending-migration entry is a technical repair on something the operator already has, not a fresh "what should this help with?" question, so this skill's own interview (Step B onward) is the wrong path for it. If the operator hasn't already brought it up, mention it plainly and hand off, for example:

> Something you already had is paused, waiting to be rebuilt safely after an update. There's a dedicated flow for exactly that, so we don't need to redesign it from scratch here.

Then hand off to the **rebuild-paused-capability** skill. Its business purpose does not change — only its technical wiring does — so the rebuild keeps the same id as the entry's `mechanism_id` rather than declaring a new one, and the entry will close itself automatically the moment the rebuilt capability is accepted for live use: you never edit `pending_migrations.json` by hand, and you never need to remember to remove the entry yourself.
