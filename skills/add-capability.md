...
2. **The next-phase skill sent them here** because the request did not match the plan. (That routing is wired elsewhere; you just need to open gracefully when it happens.)
...
*(The mechanics that physically write these updates — including writing the typed capability descriptor described below with its plan-phase identifier, and the projection that registers the high-risk class in `co-protected-workflows.md` — are wired in a sibling task. This skill's job is to drive the cascade and to surface the purpose/scope confirmations to the operator; the behavior described here is the contract that wiring implements.)*
...
*(The exact mechanics of invoking next-phase from here are wired in a sibling task. What this skill guarantees to that hand-off: the capability exists in the plan, the build progress, and the acceptance file; a typed descriptor is declared but not accepted; the Recovery Profile has named the safe trial target; and the operator has been offered building as the single recommended next step.)*
