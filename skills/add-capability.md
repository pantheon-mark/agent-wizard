...
**Also check for a pending migration.** If `agents/handoffs/pending_migrations.json` exists and is non-empty, something the operator already has is switched off and waiting. Each entry names the mechanism (its `mechanism_id`), the `reason` it is off, and a `suggested_next_step`. **Read those two fields rather than assuming the cause** — entries arrive for more than one reason and they do not all mean the same thing:
...
- **The code moved out from under an approval.** A capability's implementation changed after it was approved for live use, so the approval was switched off pending a fresh trial.
- **The operator took their approval back.** They withdrew it themselves. Nothing about the capability's code changed, and there may be nothing wrong with it at all — they simply do not want it live, or did not recognise the approval that was on record.

The entry's own `reason` says which. Do not tell the operator their code has a problem when the entry says they withdrew an approval.
