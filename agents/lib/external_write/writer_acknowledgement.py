...
Why this exists
---------------
ADR-0046's coarse fail-closed gate blocks live-enable while an unresolved
...
2. **Hash-bound.** The acknowledgement records the writer's content hash at the
   ...
   the same hash-binding discipline as ADR-0045's F-3B migration quarantine.
...
def acknowledge_writer(project_root: str,
    ...
    Fails closed, with a plain-language reason, when:
      ...
      * the confirmation contains a newline or carriage return (paste-safety --
        the same fail-closed rule ADR-0045's F-2 acceptance CLI applies, since a
