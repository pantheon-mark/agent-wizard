...
- **A trial run.** You can now run one operation, once, as a trial: a real change to your real system, made and then put straight back. Here is what a trial does. It refuses up front anything it cannot establish from what that operation's own adapter declares about itself — silence is a refusal, never a pass. It requires a limit on how much the operation may touch, and refuses to run without one. It counts what it uses against that limit; that count covers this one approved trial and is not a lifetime running total. It requires your approval on the command. What it is about to do is written and flushed to disk *before* the change is attempted, so a run interrupted at any moment leaves a record that can be found again. And it writes the proof **only** if it observed the whole round trip — the change landing, and the state coming back — and writes nothing at all otherwise. **A trial passing tells you this operation can be made and put back. It does not tell you that a later live run will be permitted.**
