...
- **A trial run.** You can now run one operation, once, as a trial. It is a real change to your real system, and it is held to the live bounds — the same limit on how much it may touch, the same running record of what it did, and your approval on the command. Whatever a live run of that same operation would be held to, a trial is held to as well, **with exactly one exception, and that exception is the point of it:** a live run needs the capability to be switched on already, and a trial does not — that is what lets a trial be the thing that earns the switch-on. So a trial passing tells you this operation can be made and put back; it does not tell you that later live runs will be permitted. What the trial is about to do is written and flushed to disk *before* the change is attempted, so a run interrupted at any moment leaves a record that can be found again. A trial that completes produces the proof acceptance asks for.
