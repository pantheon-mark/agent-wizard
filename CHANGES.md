...
**Public-facing change:** A reliability release that closes two situations you could get into and not get out of. Switching a capability on has always required a proof that it can make its change on the real system and put it back. Until now nothing you could run produced that proof — so a capability could be set up correctly and never become switchable. You can now run a single trial that produces it.

- **A trial run.** You can now run one operation, once, as a trial: a real change to your real system, made and then put straight back. Here is what a trial does. It refuses up front anything it cannot establish from what that operation's own adapter declares about itself — silence is a refusal, never a pass. It requires your approval on the command. **For an operation your system gates** — one classed as impossible to undo, as running unattended on a schedule, or as touching sensitive data, and any operation carrying no risk contract at all, which is treated as the most protected of those — a trial requires a limit on how much that operation may touch, refuses to run without one, and counts what it uses against it; that count covers this one approved trial and is not a lifetime running total. **That does not cover every operation.** In particular, one whose contract classes its changes as reversible, and which does not separately require an approved phase, is not gated here, and a trial of it enforces no limit at all. Reversible is what that setting defaults to, so it is worth knowing which classification an operation carries before leaning on a trial's limit. What the trial is about to do is written and flushed to disk *before* the change is attempted: once that record exists, an interruption at any later instant leaves a durable record that can be found again, and before it exists an interruption leaves nothing changed and no record — nothing was changed, and there is nothing to find. And it writes the proof **only** if it observed the whole round trip — the change landing, and the state coming back — and writes nothing at all otherwise. **A trial passing tells you this operation can be made and put back. It does not tell you that a later live run will be permitted.**
...
- **One behaviour deliberately changed, and it can re-block something you had already cleared.** A recorded risk-acceptance is now honoured **only** for a file that genuinely cannot be rebuilt automatically. Before this version, such a record was honoured for *any* flagged file — including one the rebuild flow could have fixed — so recording the acceptance took that file off the blocked list and its rebuild never had to happen. That was the defect. After this upgrade a record of that second kind stops counting: the file it names goes back to being blocked and shown as needing a rebuild, **and nothing tells you that a record you made has become inert.** If a file reappears as blocked after this upgrade, that is why. Records against files that genuinely need a person are unaffected.
- **Upgrading in one step works from `v0.6.9` and later.** A system still on `v0.3.0`–`v0.6.8` cannot reach this version in a single step and must be brought to `v0.6.9` or later first.
...
**Still open, named here rather than implied fixed:** if the outside system a capability writes to is named differently from the capability's own name, that capability can be switched on and still have **every** live write refused — the check that decides whether your approval covers an operation compares the capability's own name against the name of the outside system, so the two never match and there is nothing you can do to that capability to make them match; a retrial item's next step exists on disk and in the skills but is not printed by any surface; a paused hand-rolled writer's pair of marker files has a written route out but no single command that performs it; and a wrapper's guard is recognised by what it looks like, which does not establish that it will fire.
...
- **If a capability's read-only file exists but what it declares cannot be read — for example, a registration written inside a function instead of at the top level — the message you get now names that exact file**, instead of the generic message telling you the capability itself needs to be rebuilt.
...
`Source-Meta-Commit:` `9ec2a81` (private build repo; merge `5c15015`) · public repo commit `26be597`
...
`Source-Meta-Commit:` `f3e2f5c` (private build repo) · public repo commit `7e99531`
...
`Source-Meta-Commit:` `db5ec28` (private build repo) · public repo commit `407154c`
...
`Source-Meta-Commit:` `2b94173` (private build repo) · public repo commit `28ba3f3`
...
`Source-Meta-Commit:` `e662542` (private build repo) · public repo commit `d34c42a`
...
`Source-Meta-Commit:` `4a6951f` (private build repo) · public repo commit `4cc6ca9`
...
`Source-Meta-Commit:` `46b27d8` (private build repo) · public repo commit `6bf5a55`
...
`Source-Meta-Commit:` `ee9b85c` (private build repo) · public repo commit `f7f52af`
...
`Source-Meta-Commit:` `2a762b3` (private build repo) · public repo commit `747c887`
...
`Source-Meta-Commit:` `551dfc8` (private build repo) · public repo commit `29d6ec1`
...
`Source-Meta-Commit:` `fc90e44` (private build repo) · public repo commit `5342673`
...
`Source-Meta-Commit:` `e84a357` (private build repo) · public repo commit `fadba1a`
