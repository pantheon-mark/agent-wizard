...
**Ground every question in their system, balanced — do NOT read the table's wording as a generic script.** Build each question from what the operator has already told you (purpose, working definition, earlier beat answers), with a concrete example drawn from *their* system, and phrase it so both a yes and a no sound natural — present both readings so it helps them decide *without* leading. Keep it short. For example, turn the generic "Will other people use it, with their own access?" into something like "Will [the specific person they named] sign into this and use it directly, with her own access — or are you the only one working in it, and you pass along what she needs to see?" — grounded in their situation, and balanced so neither answer is the obvious one. The "Ask the user" column below is the canonical **meaning to convey plus a neutral fallback wording** — use it verbatim only when you have no context to ground from. The runtime question (2) keeps its three contrastive levels exactly; only its cadence examples may be grounded. Never pre-fill or answer a question for them — the anti-bias guardrail above still binds: examples *frame*, never *fill*, and an example that tips the operator toward one answer is leading and not permitted.
