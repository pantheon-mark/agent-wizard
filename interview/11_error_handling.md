...
- Write NOTIFICATION_VERBOSITY or THREE_STRIKES_THRESHOLD to `project_instructions.md` as wizard-runtime config (`project_instructions.md` in foundation-only mode is a foundation-doc per `_foundation_only_mode_gate.md` § 4; see step 15 close for foundation-doc voice)

DO:

- Conduct the error-handling questions and capture answers
- Append captured error-handling approach to the staging file under `## Foundation-only-mode captures > Error-handling approach` (verbosity preference + three-strikes default + plain-language description of error-handling intent)

At step 15 close, the captured error-handling approach extracts to `technical_architecture.md` § "Operational requirements" > "Error-handling approach" per `_foundation_only_mode_gate.md` § 5.
