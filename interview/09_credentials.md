...
Five further per-dependency fields are OPTIONAL typed-capability-descriptor fields (`action_class` / `risk_class` / `recovery_profile_ref` / `declared_test_target` / `blast_radius_cap`; see `dependency_projection.py`'s `ACTION_CLASSES` / `RISK_CLASSES` / `TEST_TARGETS` for the closed vocabularies). At step 09 these are normally left unset — a dependency this general capture step captures does not yet have a settled capability classification; the `add-capability` flow is what fills them in for a specific capability later. If a dependency already carries any of these five fields from an earlier derivation (an `add-capability` run, or a prior step-09 pass), **carry them through unchanged** when you re-derive the record — do not drop a field just because this step does not ask about it. An absent or unrecognized `risk_class` is always treated as the most-protected class downstream, never as safe, so leaving it unset is deliberately conservative, not a gap.

```
python3 wizard/scripts/interview_cli.py derive-field --transcript ~/claude-wizard-draft/wizard_transcript.jsonl --field EXTERNAL_DEPENDENCY_IDENTITY --sources DEP-1 --value '<JSON array of {id, name, type, roles, credential_facet?, owner_agent_id?, action_class?, risk_class?, recovery_profile_ref?, declared_test_target?, blast_radius_cap?}>'
